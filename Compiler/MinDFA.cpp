...
void MinDFA::alterChild(int oldIndex, Node *parent, int newIndex) {
    ...
    for (int i = size - 1; i > oldIndex; i--) {
        temp.push_back(children[i]);
        children.pop_back();
    }
    children.pop_back();
    children.push_back(nfaToDFA->dfaGraph[newIndex]);

    while (!temp.empty()) {
        children.push_back(temp[temp.size() - 1]);
        temp.pop_back();
    }
}
...
void MinDFA::removeFromChildren(int removedIndex, int newIndex) {
    ...
    Node *replacingNode = nfaToDFA->dfaGraph[newIndex];
...
void MinDFA::engine() {
    ...
    for (int i = 0; i < size; ++i) {
        for (int j = i + 1; j < size; ++j) {
            if (( nfaToDFA->dfaGraph[i] ==  nfaToDFA->dfaGraph[j]) &&
    ...
    Node *minDFARoot =  nfaToDFA->dfaGraph[0];/*this is the min dfa root*/
}